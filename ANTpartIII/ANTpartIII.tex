...
\chapter{Philosophies}
  \section{The Katz-Sarnak Philosophy}
    ...
    \subsection*{Families of \texorpdfstring{$L$}{L}-functions}
      ...
      That is, the average of $\mu_{k}(A)[a,b]$ for any of the matrix ensembles, in the limit of the size of the matrices, always resembles $\mu_{k}(\mathrm{CUE})[a,b]$. On the other hand, there are statistics that depend on the particular ensemble chosen. For example we can define the distribution of the $k$-th eigenphase over $G(N)$ as follows:
      \[
    ...
    \subsection*{Modeling $L$-functions by Characteristic Polynomials of Unitary Matrices}
      ...
      \]
      for some coefficients $a_{n}$. This is the analogue to a the Dirichlet series representation of an $L$-function. Of course, as $\L_{A}(s)$ is a polynomial it admits analytic continuation to $\C$. It also has a functional equation of shape $s \to \frac{1}{s}$. To see this, since $A$ is unitary
      \[
        \L_{A}(s) = (-1)^{N}\det(A)s^{N}\det(I-s^{-1}A^{-1}).
      \]
      But $\L_{A^{-1}}(s) = \L_{A^{\ast}}(s) = \conj{\L_{A}(s)}$, so the above equation can be expressed as
      \[
...
\bibliography{referenceANTpartIII}
