  ...
  \section{The Katz-Sarnak Philosophy}
    ...
    \subsection*{Families of \texorpdfstring{$L$}{L}-functions}
      ...
      \begin{center}
        ...
        \end{stabular}
      \end{center}
      Now let $G(N)$ be any of the first three compact matrix groups: $\U(N)$, $\SO(N)$, or $\mathrm{USp}(2N+1)$. Let $dA$ denote the Haar measure and for any $A \in G(N)$ lable the eigenphases as $\t_{n}$ for $1 \le n \le N$ in increasing order. One of the statistics that Katz and Sarnak investigated was the \textbf{$k$-th consecutive spacing}\index{$k$-th consecutive spacing}:
      \[
